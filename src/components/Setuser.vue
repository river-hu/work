...
export default {
  ...
  mounted() {
    this.id = this.$route.params.id;
      let params = new URLSearchParams();
          params.append("id", this.id);
      this.$api.get("home.php",params,(data)=>{
          var user = data[0];
          this.user = data[0];
          this.formValidate.name = user.name;
          this.formValidate.mail = user.email;
          this.formValidate.city = user.city;
          this.formValidate.password = '';
          this.formValidate.date = user.days;
          this.formValidate.gender = user.sex;
          this.formValidate.interest = user.liker.split(",");
          this.formValidate.desc = user.dec;        
          var that = this;
          this.uploadList = this.$refs.upload.fileList;
          this.imgName =user.header;
           this.uploadList.push({
            'url':that.url+'/img/'+that.id+'/'+user.header,
            'name':user.header,
            'status':'finished'
          })
      })
